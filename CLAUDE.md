...
- **앱에서 판정 규칙을 새로 만들기.** 피부 지표의 높음/낮음은 서버 `highlights`,
  ...
  - 예외는 이미 있는 표시 등급 두 개뿐이다 — `SkinLevel`(총점) ·
    `MetricBand`(개별 지표 60/40). 새로 만들지 말고 이 둘을 쓴다
  - **총점 등급은 `SkinLevel` 하나다.** 경계(20/40/60/80)는 서버 `SkinLevel.of` 를
    그대로 옮긴 것이고 `lib/shared/enums/skin_level.dart` 한 곳에만 있다.
    서버가 `grade` 를 주면 `fromJson`, 점수만 주는 자리(홈·기록)에서만 `fromScore` 다
  - 앱이 75/60 경계(옛 `ScoreGrade`)를 따로 쓰던 시절에는 같은 68점이 홈에서
    "보통", 리포트에서 "좋음"으로 떴다. 화면별 임계값을 다시 만들지 마라
