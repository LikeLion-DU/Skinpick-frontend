...
/// 예상 섭취량.
///
/// **확정 시안이 이 값을 화면에 올렸다**("예상 섭취량 : 보통"). 그전까지는
/// [label] 을 두지 않았다 — AI 가 사진에서 본 대략의 양이라 사용자가 실제로 먹은
/// 양과 다르고, 척도를 정의한 기준도 없어서("사진에서 관찰한 것만" 이 프롬프트
/// 규칙의 전부다) 숫자처럼 얹으면 자기 식사량의 근거로 읽힐 수 있었다.
///
/// 시안이 그 걱정을 라벨로 흡수했다. 칩 문구가 **"예상"** 으로 시작하고 매운 정도·
/// 기름기와 나란히 "AI 가 사진에서 본 것" 묶음에 들어가므로, 측정값으로 읽힐 자리가
/// 아니다. 그래서 값을 그리되 척도 이름(적음·보통·많음)만 쓰고 그램·칼로리로
/// 환산하지 않는다.
///
...
enum PortionSize {
  ...
  /// 모르면 null 이라 칩을 안 그린다. 서버 프롬프트가 "확실하지 않으면 UNKNOWN"
  /// 을 강제해서 실제로 자주 빈다 — 빈 칩을 그리면 화면이 모른다는 사실조차
  /// 말하지 못한다.
