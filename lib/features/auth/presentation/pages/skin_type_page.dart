          ...
          Expanded(
            child: GestureDetector(
              ...
              child: Pill(
                ...
                // 지나갔다.
                //
                // **기본 배율에서 344dp 아래면 다시 접힌다.** 세 알약이 폭을
                // 똑같이 나눠 갖는 한 여기가 천장이다. 그 아래까지 한 줄로 받으려면
                // 알약을 글자 길이만큼만 잡아 남는 폭을 나눠 주는 배치로 바꿔야 한다.
                //
                // 배율을 올렸을 때 접히는 것은 이 천장과 다른 이야기다 — 위에 적은
                // 정책대로 **접히는 게 맞다.** 360dp 에서 1.15 배만 되어도 두 줄이
                // 되지만, 그때는 알약이 36 → 54 로 같이 자라 글자가 잘리지 않는다.
                // 그 동작은 테스트가 잡고 있다(scale_and_state_test).
                horizontalPadding: 2,
                borderRadius: 16,
                maxLines: 2,
                color: step == current
