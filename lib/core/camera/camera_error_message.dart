...
String cameraErrorMessage(Object error, String fallback) {
  ...
  return switch (error.code) {
    ...
    'CameraAccessRestricted' =>
      '카메라 권한이 필요해요.\n$_settingsPath 에서 허용해 주세요.',
    _ => fallback,
  };
}

/// 권한을 되돌리러 갈 자리. **두 OS 가 서로 없는 경로다** — iOS 설정에는 "앱 >
/// 권한" 이 없고, Android 설정에는 앱 이름이 최상위로 올라오지 않는다.
/// 없는 경로를 안내하면 사용자는 설정을 뒤지다 포기하고 앱을 지운다.
///
/// 코드는 양쪽이 같다(`CameraAccessDenied` 셋). 갈라지는 것은 문구뿐이라
/// 여기서만 나눈다.
String get _settingsPath => defaultTargetPlatform == TargetPlatform.iOS
    ? '설정 > Skinpick > 카메라'
    : '설정 > 앱 > 권한';
