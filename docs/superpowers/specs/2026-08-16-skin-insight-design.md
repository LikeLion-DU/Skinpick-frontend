...
- **최근 생활 상태** — 수면·스트레스·운동·물 섭취를 `auth.user` 에서 읽어 각 enum의 `label` 로 표시.
...
- **오늘의 행동** — `todayActions` 를 순서 그대로 카드로.
...
### 생성 시점 스냅샷 — 알고 있어야 할 한계

인사이트는 **생성 시점의 습관을 스냅샷으로 굳혀** 저장한다 (`skin_insight.snapshot_*`).
그런데 화면의 "최근 생활 상태" 는 `auth.user` 의 **현재 값**을 읽는다. 인사이트를 본 뒤에 습관을
바꾸고 다시 들어오면, 생활 상태 칸은 새 값인데 AI 문장은 옛 값 기준인 상태가 된다.
서버에 재생성 API가 없고 만들지도 않기로 했으므로 앱이 고칠 수 있는 문제가 아니다.

그래서 두 가지를 한다.

- 생활 미설정 안내 문구를 정직하게 쓴다 — 지금 보고 있는 인사이트가 바뀌는 게 아니라
  **다음 분석부터 반영된다**는 것을 알린다.
- **데모 순서: 생활 상태를 먼저 설정하고, 그다음 촬영 → 결과 → 인사이트로 간다.**
  순서가 뒤집히면 습관 토픽이 하나도 없는 인사이트가 그 분석에 그대로 굳는다.
...
| 순서 | 저장소 | 브랜치 | 내용 |
|---|---|---|---|
| 1 | backend | (진행 중인 `feat/skin-insight-api` 위) | `ExerciseHabit` 4종 + 문서 + 테스트 |
...
백엔드는 **작업 트리가 이미 더럽다** — `feat/skin-insight-api` 에 커밋되지 않은 인사이트 API 수정이
남아 있다. 남의 작업 위에 커밋을 얹지 않는다. enum 변경만 만들어 두고 커밋 여부는 그 브랜치 주인이 정한다.
...
- 온보딩 회귀 — `_StubAdapter` 로 `PATCH /auth/me` 바디에 고민·습관 4종이 실리는지, 미선택 필드가 빠지는지.
- 위젯 — 시안 폭 402에서 인사이트 화면 오버플로. 첫 분석·인사이트 없음 상태도 같이 그린다.
- `flutter analyze && flutter test` 둘 다 통과해야 커밋한다.

## 위험

- **백엔드 인사이트 API가 아직 미머지다.** `feat/skin-insight-api` 브랜치에 커밋되지 않은 수정이 남아 있다.
  위 계약은 그 작업 트리를 실측한 것이므로, 머지 전에 응답이 더 바뀌면 fixture와 DTO를 같이 고쳐야 한다.
