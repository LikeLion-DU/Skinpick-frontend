...
- **백엔드 인사이트 API가 아직 `develop` 에 머지되지 않았다.** 커밋은 끝났고 계약은 고정됐지만
  리뷰에서 응답이 더 바뀌면 fixture와 DTO를 같이 고쳐야 한다. 실제로 리뷰 반영 커밋(`cc47d18`)이
  "빈 인사이트를 저장하지 않는다" 를 바꿔 놨고, 그게 이 설계의 안내 문구 분기를 만들었다.
