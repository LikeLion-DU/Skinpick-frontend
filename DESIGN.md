...
> **낡은 서술 (2026-08-19 확인)** — 아래 본문은 확정 시안 이전 코드를 기록한 것이라
> 지금과 어긋나는 서술이 남아 있다. 고치지 않고 여기 적어 둔다 — 문서를 통째로
> 다시 쓰는 것은 동결 뒤 작업이고, `[src:N]` 인용 번호가 본문 전체에 걸려 있다.
>
> - **`ScoreGrade`(75/60 경계)는 없어졌다.** 총점 등급은 서버가 매겨 보내고 앱은
>   `SkinLevel` 로 받기만 한다. 앱에 점수 경계표가 한 줄도 없다(§등급 표와
>   "등급 경계값(75/60)은 역산이다" 항목이 그 시절 것이다).
> - **`ScoreBadge` 위젯과 `SkinLevel.solidColor` 는 삭제됐다.** "배지가 두 벌로
>   그려진다" 는 그렇게 해소됐고, `r-badge: 10` 과 부록 11번 항목도 그 위젯을 가리킨다.
> - **지표는 4개가 아니라 5개다**(수분·유분·홍조·트러블·장벽). 마이페이지 시안이
>   갱신되면서 확정됐다.
> - 추천 화면(S08)의 `Colors.green`/`Colors.red` 는 시안 규약(GOOD/BAD)으로 바뀌었다.
...
### 상태어 명암비 — 알려진 미달 (2026-08-19 실측)

지표 막대의 상태어(`MetricBand`)는 크림 카드(`surfaceCardWarm #FEF5EB`) 위에 **10sp w600** 으로 놓인다. 10sp 는 WCAG 의 large text(18pt, 또는 14pt bold)가 아니라 **일반 텍스트**라 AA 기준이 4.5:1 이다. 네 색을 그 배경에서 실측하면:

| 상태어 | 색 | 크림 위 대비 | AA 4.5:1 | 출처 |
|---|---|---|---|---|
| 부족 | `metricLow #3A85E2` | **3.46:1** | ❌ | **앱이 정한 색**(PR #50) — 시안에 없다 |
| 좋음 | `good #6CBE46` | **2.14:1** | ❌ | 시안 |
| 보통/주의 | `primary #FF7D40` | **2.36:1** | ❌ | 시안 |
| 주의(높을수록 나쁨) | `bad #DF0011` | 4.70:1 | ✅ | 시안 |

**파랑만의 문제가 아니다.** 리뷰가 지목한 `metricLow` 는 셋 중 가장 나은 값이고, 시안 색인 초록·오렌지가 더 낮다. 그래서 파랑만 고치면 **앱이 정한 색만 기준을 넘고 시안 색은 미달로 남는** 상태가 된다 — 팔레트가 그 순간 두 규칙으로 갈린다.

**이번 릴리스에서 색을 바꾸지 않았다.** 셋 중 둘이 시안 색이고, 이 저장소에는 [Figma 변수 원본이 없어](#colors) 디자이너 확인 없이 시안 색을 옮기면 그게 곧 앱이 만든 디자인이다.

바꾸게 되면 참고할 최소값 — 색상·채도를 유지하고 명도만 낮춰 4.5:1 에 닿는 지점은 `metricLow` 가 **`#1F70D4`**(4.50:1) 다. 다만 위 이유로 **팔레트 전체를 디자이너와 함께 정하는 쪽**을 권한다. 대안으로 색을 그대로 두고 글자를 키우거나(14sp bold 이상이면 large text 기준 3:1 로 셋 중 파랑만 통과) 상태어에 아이콘을 더해 색 단독 전달을 피하는 방법이 있다.
...
피부 결과의 "복합성 피부" 카드 [src:25]. **radius 16 + `primary` 0.6px 테두리**로 기본 카드와 층을 가른다. 제목 옆 민감도 배지는 **홍조가 60 이상일 때만** 단다 — 근거 없이 달면 "높음"이 안 뜨는 날 이 배지의 신뢰가 같이 사라진다.

지표 4종은 57px 원 + 24px 아이콘 + 라벨 + 상태어다. 상태어의 색은 지표 방향에 따라 다르게 계산한다 — 높을수록 좋은 지표(수분·피부결)는 40 미만이 "부족"(파랑 `#3a85e2`), 높을수록 나쁜 지표(유분·홍조)는 60 이상이 "주의"(빨강 `#fa6154`)다.
